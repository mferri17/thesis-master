...
Since we mainly focus on improving the FrontalNet model explained in section \ref{subsec:sota-dario}, we firstly need to understand the existing environment in which the drone has been originally taught to fly, physically located in Manno\footnote{now relocated to Lugano} (Switzerland) at \gls{idsia}.

This chapter aims to provide a generic view of our system, its main components and how they interact for flying and controlling the drone. Then, we present tools, libraries and dataset we are working with for achieving our objective.
...
The original work from \cite{mantegazza2019visionbased} is written in Python and based on ROS, TensorFlow 1, and Keras. This section presents tools and software used to conduct our research for improving the existing system from a machine learning point of view.

The source code is written in Python 3. First experiments were carried out with Jupyter Notebooks via Google Colab on a GPU-accelerated runtime, while the final code is provided as Python scripts to be executable on a custom machine.
